...
test("MY EQP 자설비 조회는 clicked_history_defect용 line과 STEP을 함께 생성한다", () => {
  const clickedAt = "2026-07-17T13:00:00+09:00"
  const record = buildClickedCategoryHistoryRecord({
    app: "self",
    lineId: "P1L",
    filePaths: [
      "/appdata/abnormal_trend/pic/erd/2026-07-17/SDWT-1/MAIN_ETCH/V1/PPID-1/A/TEMP/10@001/EQP-1.png",
    ],
    grades: ["A"],
    defectSelectStep: "MAIN_ETCH",
    clickedAt,
    knoxId: "user1",
  })

  assert.deepEqual(record.defectHistory, {
    lineName: "P1L",
    selectStep: "MAIN",
  })
  assert.equal(record.updateDate, clickedAt)
  assert.equal(record.knoxId, "user1")
})
