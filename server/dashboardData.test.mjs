...
test("같은 날짜의 여러 시각 파일은 합쳐서 고유조합을 중복 제거한다", () => {
  const sameDayFiles = [
    datedRows[0],
    {
  ...
  assert.equal(payload.summary.totalAbnormalCount, 5)
  assert.equal(payload.lineSummary.find((row) => row.lineId === "P1").totalCount, 4)
  assert.equal(payload.lineSummary.find((row) => row.lineId === "P2").totalCount, 1)
})
...
test("기본 조회 기간은 최신 파일을 포함한 최근 7일이며 잘못된 날짜를 거부한다", () => {
