...
| Self Equipment 조회 | [4. 설비별 SPEC내 이상감지](USER_MANUAL.md#4-설비별-spec내-이상감지), `03`~`07` 이미지 | `/self-equipment`, alias `/fdc_trend/self-equipment` | [self-equipment.md](../features/self-equipment.md) | [DF-SELF-01~03](../system/data-flow.md#10-self-equipment-및-이상-데이터-흐름) | `/api/self-equipment-data`, `/api/my-eqp-equipment-data`, `/api/erd-scatter-data`; 별도 JSON Schema 없음 | [파일 누락·권한](../operations/troubleshooting.md#8-appdata-파일-누락-또는-권한-오류) |
...
| Self Equipment | 설비별 SPEC내 이상감지 | `/self-equipment` | `/fdc_trend/self-equipment` | `Confirmed` | `routes.jsx:16-19`; `L0SpiderHomePage.jsx:11-19` |
...
해결 이력: `MAN-M05`는 현재 [사용자 매뉴얼](USER_MANUAL.md)의 오류 안내가 발생 시각·선택 조건·문의 코드 기록으로 변경되어 미해결 목록에서 제외했다. 실제 브라우저 화면은 이번 문서 수정에서 재현하지 않았다.
