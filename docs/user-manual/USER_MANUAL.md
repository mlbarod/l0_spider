...
6. [My EQP 등록](#6-my-eqp-등록)
7. [동일성 이상감지](#7-동일성-이상감지)
8. [공통부 이상감지](#8-공통부-이상감지)
9. [사용자 매뉴얼 화면과 외부 SPIDER](#9-사용자-매뉴얼-화면과-외부-spider)
10. [검색·필터와 결과 확인 공통 방법](#10-검색필터와-결과-확인-공통-방법)
11. [데이터 등록·수정·삭제](#11-데이터-등록수정삭제)
12. [오류 메시지와 해결 방법](#12-오류-메시지와-해결-방법)
13. [자주 묻는 질문](#13-자주-묻는-질문)
14. [사용자 주의사항](#14-사용자-주의사항)
...
| My EQP 등록 | 집중 모니터링할 개인 설비 조건을 등록·삭제 | 구현 완료 | Line, SDWT, PRC Group, EQP, 기간, Comment |
...
| FDC Hard Limit추천 | FDC 분포 기반 Hard Limit 추천 서비스로 이동 | 운영중, 외부 서비스 | 새 웹페이지에서 확인 |
...
2. **SDWT**에서 분임조를 선택합니다. SKIP한 결과만 확인하려면 `SKIP LIST`, 등록한 개인 설비를 보려면 `MY EQP`를 선택합니다.
...
| SDWT | 필수 | 조회할 분임조, `MY EQP` 또는 `SKIP LIST` | `DEMO_SDWT` |
...
My EQP 등록은 자설비 이상감지에서 자주 확인할 설비를 개인 조건으로 저장하는 화면입니다. 등록된 조건이 유효한 동안 자설비 이상감지의 SDWT 필터에 `MY EQP`가 표시됩니다.

### 6.1 My EQP 조건 등록

1. 메인 화면에서 **My EQP 등록** 카드를 클릭합니다.
2. **Line Name**에서 모니터링할 라인을 선택합니다.
3. **SDWT**에서 해당 라인의 분임조를 선택합니다.
4. **PRC Group**을 선택합니다.
5. **EQP**에서 설비를 선택합니다. 개별 EQP는 복수 선택할 수 있습니다.
6. 전체 설비를 등록하려면 `ALL`을 선택합니다. `ALL`과 개별 EQP는 동시에 선택할 수 없습니다.
7. 필터 아래 **선택 조건**에서 Line, SDWT, PRC Group, EQP가 맞는지 확인합니다.
8. **모니터링 기간**에 1 이상의 일수를 입력합니다.
9. 필요한 경우 **Comment**에 등록 사유나 참고사항을 90자 이내로 입력합니다.
10. **My EQP 저장**을 클릭하고 저장 완료 메시지에서 저장 행 수를 확인합니다.

복수 EQP 또는 `ALL`을 저장하면 실제 EQP 1개당 DB 한 행으로 등록됩니다. 현재 사용자는 접속 IP를 기준으로 확인하며 사용자 정보가 없으면 접속 IP가 사용자 식별값으로 저장될 수 있습니다.

### 6.2 등록 조건 확인과 삭제

Line Name을 선택하면 **등록된 My EQP 조건**에 해당 Line의 등록 내역이 표시됩니다. 각 카드에서 다음 내용을 확인할 수 있습니다.

- Line, SDWT, PRC Group, 등록 EQP 목록
- 등록 시점과 만료 시점
- 모니터링 기간과 Comment
- `모니터링 중` 또는 `기간 만료` 상태

등록 조건을 제거하려면 카드 오른쪽의 **삭제**를 누르고 확인 창에서 다시 **삭제**를 누릅니다. 복수 EQP로 함께 등록한 조건은 해당 저장 시점의 EQP 행이 모두 삭제되므로 대상을 먼저 확인하십시오.

### 6.3 자설비 이상감지에서 조회

1. 자설비 이상감지 App을 열고 My EQP를 등록한 **Line Name**을 선택합니다.
2. **SDWT**에서 `MY EQP`를 선택합니다. 유효한 등록 조건이 없으면 이 항목은 표시되지 않습니다.
3. Sensor Grade는 등록 설비에 존재하는 등급 전체가 기본 선택됩니다. 필요하면 원하는 등급만 남깁니다.
4. **STEP**을 선택합니다.
5. **eqp_ch**, **sensor**, **ch_step**을 차례로 선택합니다. 각 단계에서 `ALL`이 보이면 전체 조건을 선택할 수 있습니다.
6. 아래 Scatter chart에서 등록한 SDWT·EQP에 해당하는 이상건을 확인합니다.

조회 대상은 등록한 Line의 SDWT 경로와 EQP를 기준으로 매칭됩니다. EQP 이름의 대소문자, 공백, `_`, `-`, `.png` 표기 차이는 조회 시 정규화됩니다.

### 6.4 모니터링 기간 만료

유효기간은 `등록 시점 + 입력한 기간(일)`로 계산됩니다. 만료된 조건은 DB 이력에서 자동 삭제되지는 않지만 자설비 이상감지의 `MY EQP` 조회 대상에서는 제외됩니다. 화면을 열어 둔 경우에도 주기적으로 유효기간을 다시 확인하므로, 활성 조건이 모두 만료되면 SDWT 필터의 `MY EQP`가 자동으로 사라집니다.
...
| My EQP 저장 | My EQP 등록 | EQP 1개당 한 행 저장 | Line·SDWT·EQP·기간 확인 후 저장 |
| My EQP 삭제 | My EQP 등록 | 선택 등록 조건의 EQP 행 삭제 | 삭제 확인 창에서 대상 재확인 |
...
| 등록된 SDWT·EQP와 일치하는 자설비 이상건을 찾지 못했습니다. | 활성 My EQP 조건은 있으나 현재 이상건에 일치 대상이 없음 | 등록 Line·SDWT·EQP와 현재 이상건 생성 여부 확인 |
| My EQP 기준정보 DB 저장/조회/삭제에 실패했습니다. | My EQP DB 작업 실패 | DB 연결, 테이블 권한, 접속자 식별 정보를 관리자에게 전달 |
