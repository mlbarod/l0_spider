...
4. [설비별 SPEC내 이상감지](#4-설비별-spec내-이상감지)
...
| 메뉴 | 용도 |
| --- | --- |
| 설비별 SPEC내 이상감지 | 설비·센서별 FDC 이상 추이와 차트 확인 |
| Mailing Report 및 My EQP 등록 | 메일 수신 조건과 모니터링 설비·열람자 등록 |
| Chart Mailing 게시판 | 차트 메일 발송 건과 당시 이미지 조회, 진행중·완료 관리 |
...
아래의 **라인별 이상 건수**, **일자별 이상 추이**, **라인별 상세 현황**에서 Line별 분포를 확인할 수 있습니다. 추이는 `10일`, `30일`, `90일`, `180일`로 바꿀 수 있으며, 상세 현황의 **상세** 버튼은 해당 Line의 설비별 SPEC내 이상감지로 이동합니다.
...
설비별 SPEC내 이상감지는 최신 ERD 데이터를 산점도로 확인하는 화면입니다.
...
1. 설비별 SPEC내 이상감지에서 해당 **Line Name**을 선택합니다.
...
- Line·SDWT·Grade 조건을 쿼리에 담아 설비별 SPEC내 이상감지 주소로 이동하는 LINK
...
### 7.1 Chart Mailing 게시판

메인 화면의 **Chart Mailing 게시판**에서 차트 메일 발송 건을 조회합니다. DB 연동 활성화 이후 보내는 메일부터 자동 등록되며, 제목을 누르면 발신자·수신인·코멘트와 발송 당시 PNG 원본을 확인할 수 있습니다. 업무 상태와 제목·발신자·차트 정보로 검색할 수 있습니다.

로그인한 사용자는 누구나 **완료 처리** 또는 **진행중으로 변경**을 할 수 있습니다. 처리 코멘트는 선택 사항이며 변경자·시각·내용이 이력에 남습니다. 업무 상태 변경은 추가 메일을 발송하지 않습니다.

메일 결과와 업무 상태는 별개입니다. **처리중 / 결과 확인 필요** 또는 **결과 확인 필요**인 건은 수신 여부를 먼저 확인하세요. 게시판에서 자동 재발송하지 않습니다. 관리자가 DB 준비를 완료하기 전에는 준비 안내가 표시됩니다.
