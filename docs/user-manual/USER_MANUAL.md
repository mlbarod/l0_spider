...
7. [Mailing Report 등록과 확인](#7-mailing-report-등록과-확인)
...
| My EQP 등록 | 집중 모니터링할 설비 등록 및 공개 범위 관리 |
| 동일성 이상감지 | 동일 조건의 분석 완료 이미지 비교 |
| 공통부 이상감지 | 공통부 이상 이미지와 설비 비교 차트 확인 |
| Mailing Report 수신인 등록 | Line·SDWT별 메일 수신 조건 등록 |
| 사용자 메뉴얼 | 현재 문서 확인 |
...
- **ch_step 전체보기**: 해당 EQP의 모든 ch_step 차트를 펼칩니다.
...
My EQP는 자주 확인할 Line·SDWT·EQP를 일정 기간 개인 또는 전체 공개 조건으로 등록하는 기능입니다.

### 5.1 등록 방법

1. **Line Name → SDWT → PRC Group → EQP** 순서로 선택합니다.
2. EQP는 복수 선택할 수 있으며 `ALL`은 단독 선택입니다.
3. **모니터링 기간**에 1 이상의 일수를 입력합니다.
4. 필요하면 90자 이내의 **Comment**를 입력합니다.
5. 다른 사용자도 보게 하려면 저장 버튼 왼쪽의 **전체 공개**를 선택합니다.
6. **My EQP 저장**을 누릅니다.

`전체 공개`를 선택하지 않으면 등록자만 볼 수 있습니다. 선택하면 다른 사용자도 등록 목록과 자설비 이상감지의 `MY EQP`에서 볼 수 있습니다.
...
`MY EQP`에는 유효기간이 남은 내 등록 건과 전체 공개 건이 합쳐져 표시됩니다. 만료된 조건은 DB에서 자동 삭제되지는 않지만 자설비 조회에서는 제외됩니다.
...
**Line Name → SDWT → Sensor → ch_step** 순서로 선택하면 STEP별 분석 완료 이미지가 표시됩니다. `ch_step`의 `ALL`을 선택하면 가능한 모든 이미지가 조회됩니다.

![동일성 이상감지](images/08-matching-anomaly.png)
...
## 7. Mailing Report 등록과 확인

### 7.1 수신 조건 등록

1. **Line Name**을 선택합니다.
2. SDWT를 복수 선택하거나 `ALL`을 선택합니다.
3. 수신인의 `knox_id`를 확인합니다. 기본값은 현재 접속자이며 직접 변경할 수도 있습니다.
4. **Mailing 기능 등록**을 누릅니다.

Grade는 정책에 따라 `A`, `B`, `D`, `M`, `N`으로 고정됩니다. 아래 **등록된 Mailing 조건**에서 Line·SDWT·Grade별 조건과 호출 URL을 확인할 수 있습니다. **Line 삭제**는 해당 수신인에게 등록된 Line의 모든 SDWT·Grade 조건을 삭제합니다.
...
| My EQP 저장·삭제 | 개인 또는 공개 모니터링 조건 변경 |
| Mailing 등록·Line 삭제 | 메일 수신 조건 변경 |
...
**My EQP 전체 공개와 나만 보기의 차이는 무엇인가요?**<br>
나만 보기는 등록자에게만, 전체 공개는 모든 사용자의 My EQP 목록과 자설비 `MY EQP`에 표시됩니다.
...
- `ALL`, 전체 공개, 일괄 SKIP과 Line 삭제는 여러 데이터에 영향을 줄 수 있습니다.
