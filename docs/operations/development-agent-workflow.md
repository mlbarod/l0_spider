...
> 문서 상태: `Draft / Stage 1`<br>
> 기준 branch: `main`<br>
> 기준 commit: `9860fec`<br>
> 작성일: 2026-08-02<br>
> 적용 상태: custom agent 정의 작성 완료, 직접 호출·권한·행동 검증은 `Not Run`, 프로젝트 최상위 의무는 `Not Active`
...
현재는 도입 계획의 1단계다. 역할 기준과 custom agent 파일은 존재하지만 다음 항목은 아직 활성화하거나 확인하지 않았다.

- 모든 코드 변경에 대한 의무 호출
- 새 세션에서의 custom agent 발견 여부
- 부모 세션 override를 포함한 실제 읽기 전용 권한
- 호출 전후 무변경과 표준 출력 준수
- 시범 운영과 `AGENTS.md` 최상위 의무 반영

따라서 1~3단계에서는 사용자가 승인한 명시적 호출에만 이 workflow를 적용한다. 2단계 직접 호출 검증과 3단계 시범 운영을 통과한 뒤 4단계에서만 `AGENTS.md`에 의무 절차를 연결한다.
...
- 운영 DB, `/appdata`, 실제 mail·recipient, secret, service와 network를 승인 없이 변경하지 않는다.
...
- 메인 에이전트가 허용한 비파괴적 명령과 synthetic data만 사용한다.
...
5. 검증 에이전트에 귀속되는 변경이 하나라도 생기면 해당 검증은 `Fail`이다.
...
- 2단계: 새 세션 발견, 명시적 호출, 실제 권한·무변경과 출력 형식을 검증한다.
- 3단계: 승인된 사례에서 명시적 시범 운영하고 시간·오탐·결함 발견을 측정한다.
